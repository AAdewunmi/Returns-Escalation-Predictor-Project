...
def test_generate_synthetic_rows_produces_expected_schema_and_labels() -> None:
    ...
    assert all(row["item_category"] in {"apparel", "electronics", "homeware", "beauty"} for row in rows)
    assert all(
        row["return_reason"] in {"damaged", "wrong_item", "wrong_size", "not_as_described", "changed_mind"}
        for row in rows
    )
